...
function handleSortApplied(sort) {
  ...
  fetch().then(() => {
    ...
    each(stages.value, stage => update(stage))
  })
}
...
async function loader(columnId, $state) {
  let column = stages.value[columnId]
  ...
  if (stage.cards.length === 0) {
    ...
    stages.value[stage.id] = Object.assign(
      stages.value[stage.id],
      omit(stage, ['cards', 'summary'])
    )

    stage.cards.forEach(card => {
      stages.value[stage.id].cards.push(card)
    })
...
async function fetch(refresh = true) {
  ...
  try {
    ...
    data.forEach(stage => {
      stage.page = refresh ? stages.value[stage.id]?.page || 1 : 1
      stages.value[stage.id] = omit(stage, ['summary'])

      // Add the infinity state again to the stage and resume it.
      nextTick(() => {
        setTimeout(() => {
          if (originalInfinityState.hasOwnProperty(stage.id)) {
            stages.value[stage.id].infinityState =
              originalInfinityState[stage.id]
            stages.value[stage.id].infinityState.resume()
...
function handleColumnUpdatedEvent(event) {
  update(stages.value[event.columnId])
}
