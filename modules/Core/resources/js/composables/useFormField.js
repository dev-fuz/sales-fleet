...
export function useField(value, field, formId, isFloating, options = {}) {
  ...
  const isDirty = computed(() => {
    ...
    if (Array.isArray(value.value) && Array.isArray(realInitialValue.value)) {
      return !isEqual(value.value.sort(), realInitialValue.value.sort())
    }
  ...
  function setInitialValue() {
    if (options.setInitialValue) {
      ...
      value.value = !isNil(field.value.value) ? field.value.value : ''
    }
