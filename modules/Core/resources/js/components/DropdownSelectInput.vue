  ...
  <IDropdown items-class="max-w-xs sm:max-w-sm" v-bind="$attrs" :full="false">
    <template #toggle="{ disabled, noCaret, toggle }">
      <slot :label="toggleLabel" :toggle="toggle">
        ...
        >
          ...
          <Icon v-if="!noCaret" icon="ChevronDown" class="ml-1 mt-px h-5 w-5" />
        </button>
      </slot>
    </template>
    <div class="overflow-y-auto py-1" :style="{ maxHeight: maxHeight }">
      ...
      <IDropdownItem
        ...
        :class="
          typeof item == 'object' && Object.hasOwn(item, 'class') && item.class
            ? item.class
            : null
        "
        :icon="typeof item == 'object' && item.icon ? item.icon : null"
        :prepend-icon="
          typeof item == 'object' && Object.hasOwn(item, 'prependIcon')
            ? item.prependIcon
            : false
        "
        :disabled="
