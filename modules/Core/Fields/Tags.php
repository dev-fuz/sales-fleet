...
class Tags extends Field
{
    ...
    public function __construct($attribute = 'tags', $label = null)
    {
        ...
        $this->withDefaultValue([])
            ->provideSampleValueUsing(fn () => 'Tag1, Tag 2')
            ...
            ->displayUsing(fn ($model) => (string) $this->resolve($model)->pluck('name')->implode(', '))
    ...
    public function indexColumn(): MorphToManyColumn
    {
        return tap(new MorphToManyColumn(
            ...
            $column->select(['type', 'display_order', 'swatch_color'])->width('270px')->minWidth('270px');
        });
    }
