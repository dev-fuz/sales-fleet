<x-core::layouts.skin>
    ...
    <div class="contenteditable-view h-full min-h-screen">
        <document-public-view :requires-signature="{{ $document->requires_signature ? 'true' : 'false' }}"
            ...
            accept-thank-you-message="{{ clean($document->brand->config['document']['accepted_thankyou_message']) }}"
            sign-thank-you-message="{{ clean($document->brand->config['document']['signed_thankyou_message']) }}"
            signature-bound-text="{{ $document->brand->config['signature']['bound_text'] }}"
