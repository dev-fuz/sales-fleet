            ...
            <template #actions>
              <div class="inline-block">
                <AssociationsPopover
                  ...
                  :associated="document.associations"
                >
                  <template #after-record="{ record, selected, title }">
                    <span
                    ...
                    >
                      <Icon
                        v-if="record.is_primary_associated && selected"
                        icon="CodeBracket"
...
function syncAssociations(data) {
  ...
  Innoclapps.request()
    ...
    .then(({ data }) => {
      ...
      Object.keys(data.associations).forEach(resourceName => {
        data.associations[resourceName].forEach(record => {
          if (!document.value.associations[resourceName]) {
            document.value.associations[resourceName] = []
          }

          let currentIndex = document.value.associations[
            resourceName
          ].findIndex(r => r.id === record.id)

          if (currentIndex === -1) {
            document.value.associations[resourceName].push(record)
          } else {
            document.value.associations[resourceName][currentIndex] = record
          }
        })
      })
