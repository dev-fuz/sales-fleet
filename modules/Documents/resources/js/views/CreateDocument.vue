          ...
          <template #actions>
            <div class="inline-block">
              <AssociationsPopover
                ...
                :associateable="viaResource ? viaResourceRecord : undefined"
                :custom-selected-records="customAssociationsValue"
              >
                <template
                  #after-record="{ index, selected, isSearching, title }"
                >
                  <span
                  ...
                  >
                    <Icon
                      v-if="index === 0 && selected && !isSearching"
                      icon="CodeBracket"
...
function removeAssociation(id, resourceName) {
  let index = findIndex(customAssociationsValue.value[resourceName], ['id', id])

  if (index !== -1) {
    customAssociationsValue.value[resourceName].splice(index)
  }
}

/** Add custom association to the form */
function addAssociation(association, resourceName) {
  if (!form.associations.hasOwnProperty(resourceName)) {
    form.associations[resourceName] = []
  }

  if (!customAssociationsValue.value.hasOwnProperty(resourceName)) {
    customAssociationsValue.value[resourceName] = []
  }

  if (!find(form.associations[resourceName], ['id', association.id])) {
    form.associations[resourceName] = [association.id]
    customAssociationsValue.value[resourceName].push(association)
  }
}
